        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="max-width:600px;">
                ...
                <select name="category" class="input-custom">
                    ...
                    <option value="electronicos" {{ old('category') == 'electronicos' ? 'selected' : '' }}>Electrónicos</option>
                    <option value="ropa" {{ old('category') == 'ropa' ? 'selected' : '' }}>Ropa</option>
                    <option value="calzado" {{ old('category') == 'calzado' ? 'selected' : '' }}>Calzado</option>
                    <option value="linea_blanca" {{ old('category') == 'linea_blanca' ? 'selected' : '' }}>Linea Blanca</option>
                </select>

                <textarea name="description" class="input-custom" rows="4"
                          placeholder="Descripcion" required>{{ old('description') }}</textarea>
